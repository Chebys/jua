...
可以划分为[初等表达式](#初等表达式)、[二元表达式](.md#二元表达式)、[赋值表达式](#赋值表达式)。
...
可以提供值的序列进行声明。提供的值可以多于[声明项](#声明项)的个数，此时若带有[星号表达式](.md#星号表达式)，则将多余值构成数组并赋予星号表达式中的变量。
...
声明项由[可声明体](#可声明体)（通常是[变量名](.md#变量名)）和初始值（可选）组成。
...
第二种形式表示自动添加初始值。
* 具体来说，若未显式规定初始值，则将初始值设为 `null`；若可声明体为[解构对象](#解构对象) / [解构数组](.md#解构数组)，则递归地对子声明项自动添加初始值。
